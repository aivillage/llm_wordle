...
class OAuth2PasswordBearerWithCookie(OAuth2):
    ...
    async def __call__(self, request: Request) -> Optional[str]:
        authorization: str = request.cookies.get(admin_settings.COOKIE_NAME)
...
def create_access_token(data: dict, expires_delta: timedelta | None = None):
    ...
    encoded_jwt = jwt.encode(to_encode, admin_settings.SECRET_KEY, algorithm=admin_settings.ALGORITHM)
    return encoded_jwt
...
async def get_current_user(token: Annotated[str, Depends(oauth2_scheme)]):
    ...
    try:
        payload = jwt.decode(token, admin_settings.SECRET_KEY, algorithms=[admin_settings.ALGORITHM])
...
async def login_for_access_token(
    ...
    access_token_expires = timedelta(minutes=admin_settings.ACCESS_TOKEN_EXPIRE_MINUTES)
